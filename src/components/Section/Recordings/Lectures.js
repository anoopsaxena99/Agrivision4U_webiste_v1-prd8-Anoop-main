...
function Layout({ courseId, chapterId }) {
  ...
  const searchHandler = (filtered) => {
    if (!filtered.length)
      return toast("No Matching Lecture", {
        position: "top-right",
        autoClose: 1000,
  ...
  if (recents) {
    var high = Math.ceil(recents.length / 9);
  }

  useEffect(() => {
    if (recents)
      setState(recents.slice((pagenumber - 1) * 9, (pagenumber - 1) * 9 + 9));
